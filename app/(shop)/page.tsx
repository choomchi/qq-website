...
const SECTIONS = [
  ...
  { slug: "كودك-و-نوجوان-آفرينگان", title: "آخرین عناوین انتشارات آفرینگان", bg: "light",   cardVariant: "default" },
  { slug: "هيلا",                    title: "آخرین عناوین انتشارات هیلا",      bg: "dark",    cardVariant: "detail"  },
    ...
    <main>
      ...
      {SECTIONS.slice(0, 4).map(({ slug, title, bg, cardVariant }) => (
        <Suspense key={slug} fallback={<CategorySectionSkeleton bg={bg} />}>
          <CategorySection slug={slug} title={title} bg={bg} cardVariant={cardVariant} />
        </Suspense>
      ))}
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-8">
        ...
        <div className="flex gap-3">
          {Array.from({ length: 7 }).map((_, i) => (
            <div key={i} className="w-[calc(100%/7-10px)] shrink-0 min-w-32 rounded-xl bg-muted animate-pulse aspect-square" />
          ))}
        </div>
      </div>
    </div>
